...
namespace Phat
{
    public abstract class Actor : IActorEventSubscriptionProvider, ILocatable, IEventPublisher, IActorFactoryProvider, ILatentFunctionProvider
    {
        ...
        private readonly List<AnimationClockController> _animationControllers;
        ...
        public Actor()
        {
            ...
            this._animationControllers = new List<AnimationClockController>();
        }
        #endregion
        ...
        {
            this._animationControllers.Add(storyboard.Begin(this._processRunner, this));
        ...
        public void AnimateProperty(String property, Object fromValue, Object toValue, TimeSpan duration, Boolean shouldAutoRepeat)
        {
            var storyboard = new Storyboard
            {
                ShouldAutoRepeat = shouldAutoRepeat,
        ...
        {
            foreach (var c in _animationControllers)
            {
                c.Stop();
            }
        }
        ...
        {
            AnimateProperty(property, fromValue, toValue, duration, false);
        }
        ...
        {
            this.OnAttached(actor, Vector2.Zero);
        }
        ...
        protected virtual void OnAttached(Actor actor, Vector2 offset)
        {
            ...
            actor.AddAttachment(this);
            _attachmentOffset = offset;
        }

        private List<Actor> _attachedActors = new List<Actor>();

        protected IEnumerable<Actor> AttachedActors { get { return _attachedActors; } }

        private void AddAttachment(Actor actor)
        {
        ...
        public void Dettach()
        {
            ...
            if (this._attachedActor != null)
                this._attachedActor.Dettach(this);
        }

        public void Dettach(Actor actor)
        {
            if (this._attachedActors.Contains(actor))
