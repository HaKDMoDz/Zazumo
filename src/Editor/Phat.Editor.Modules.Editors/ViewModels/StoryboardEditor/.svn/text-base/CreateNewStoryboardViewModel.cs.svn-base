...
{
    [Export]
    public class CreateNewStoryboardViewModel : NotificationObject
    {
        ...
        public CreateNewStoryboardViewModel(IPackageRepository packageRepository, IEventAggregator eventAggregator)
        {
        ...
        private void CreateDefaultName()
        {
            ...
            while (true)
            {
                String name = String.Format("{0}.{1}.{2}{3}", PackageName.Replace(' ', '_'), "Storyboards", "Storyboard", uniqueNameNumber);
        ...
        public void OkExecute()
        {
            ...
            a.Type = EditorAssetTypes.Storyboard;
            a.Key = this.AssetName;
            a.Id = Guid.NewGuid();

            this._packageRepository.Assets.Add(a);
            _packageRepository.SaveAssetData(a.Id, new Storyboard() { Key = this.AssetName });
