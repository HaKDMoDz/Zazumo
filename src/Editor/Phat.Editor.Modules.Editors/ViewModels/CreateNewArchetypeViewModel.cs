...
{
    [Export]
    public class CreateNewArchetypeViewModel : NotificationObject
    {
        ...
        public CreateNewArchetypeViewModel(IPackageRepository packageRepository, IEventAggregator eventAggregator)
        {
        ...
        private void CreateDefaultName()
        {
            ...
            while (true)
            {
                String name = String.Format("{0}.{1}.{2}{3}", PackageName.Replace(' ', '_'), "Archetypes", "Archetype", uniqueNameNumber);
        ...
        public void OkExecute()
        {
            ...
            a.Type = EditorAssetTypes.Archetype;
            a.Key = this.AssetName;
            a.Id = Guid.NewGuid();

            this._packageRepository.Assets.Add(a);
            _packageRepository.SaveAssetData(a.Id, new ArchetypeResource() { Key = this.AssetName, Data=null });
