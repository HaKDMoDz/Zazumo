...
{
    [Export]
    public class CreateNewWorldViewModel : NotificationObject
    {
        ...
        public CreateNewWorldViewModel(IPackageRepository packageRepository, IEventAggregator eventAggregator)
        {
        ...
        private void CreateDefaultName()
        {
            ...
            while (true)
            {
                String name = String.Format("{0}.{1}.{2}{3}", PackageName.Replace(' ', '_'), "Worlds", "World", uniqueNameNumber);
        ...
        public void OkExecute()
        {
            ...
            a.Type = EditorAssetTypes.World;
            a.Key = this.AssetName;
            a.Id = Guid.NewGuid();

            this._packageRepository.Assets.Add(a);
            _packageRepository.SaveAssetData(a.Id, new WorldResource() { Key = this.AssetName, Objects = new IWorldObject[0] });
